 ...
 * Version 9.0 - Refactored mit zentralem Renderer und klarer Trennung von Logik und Darstellung.
 * @since 6.0
 */
class CSV_Import_Pro_Admin {
    ...
    public function register_admin_menu(): void {
        ...
        add_menu_page(
            ...
            __( 'CSV Importer Pro', 'csv-import' ),
            'edit_pages',
            'csv-import', // Slug des Dashboards als Hauptseite
            [ $this, 'display_main_page' ],
            'dashicons-database-import',
            25
        );

        // Alle Untermenüs definieren
        $submenus = [
            'main' => [
                'parent_slug' => 'csv-import',
                'page_title'  => __( 'CSV Import Dashboard', 'csv-import' ),
                'menu_title'  => __( 'Dashboard', 'csv-import' ),
                'capability'  => 'edit_pages',
                'menu_slug'   => 'csv-import',
    ...
    private function render_page(string $template_file): void {
        ...
        // POST-Anfragen verarbeiten, bevor Daten gesammelt werden
        ...
        // Template laden und Daten extrahieren
        extract($data);
    ...
    /**
     * Verarbeitet alle POST-Anfragen basierend auf der aktuellen Seite.
     */
    ...
    private function gather_view_data(string $template_file): array {
        ...
        // Globale Daten, die auf vielen Seiten benötigt werden
        ...
        switch ($template_file) {
            ...
            case 'page-profiles.php':
                ...
                break;
            case 'page-logs.php':
                if (class_exists('CSV_Import_Error_Handler')) $data = array_merge($data, $this->prepare_logs_data());
                break;
            case 'page-debug.php':
    ...
    /**
     * Verarbeitet die Anfrage des intelligenten Template-Generators.
     */
    private function handle_template_generator_form(): array {
        ...
        if (class_exists('CSV_Import_Template_Manager')) {
            ...
            $message = sprintf(
                'Template "%s" erfolgreich erstellt. <a href="%s" class="button button-small">Jetzt bearbeiten</a>',
    ...
    private function handle_scheduling_form(): array {
        if (!isset($_POST['_wpnonce']) || !wp_verify_nonce($_POST['_wpnonce'], 'csv_import_scheduling')) {
            return ['action_result' => ['success' => false, 'message' => 'Sicherheitscheck fehlgeschlagen.']];
        }

        $action = sanitize_key($_POST['action']);
        if ($action === 'schedule_import' && class_exists('CSV_Import_Scheduler')) {
            ...
            if(is_wp_error($schedule_result)) {
                return ['action_result' => ['success' => false, 'message' => 'Scheduling fehlgeschlagen: ' . $schedule_result->get_error_message()]];
            }
            return ['action_result' => ['success' => true, 'message' => 'Geplanter Import wurde erfolgreich aktiviert!']];
        }

        if ($action === 'unschedule_import' && class_exists('CSV_Import_Scheduler')) {
            CSV_Import_Scheduler::unschedule_import();
            return ['action_result' => ['success' => true, 'message' => 'Geplanter Import wurde deaktiviert.']];
        }
        
        // Hier könnte noch die Benachrichtigungs-Logik rein
    ...
    private function handle_backup_form(): array {
        if (isset($_POST['rollback_session']) && wp_verify_nonce($_POST['_wpnonce'] ?? '', 'csv_import_rollback')) {
            ...
            if (class_exists('CSV_Import_Backup_Manager')) {
                return ['rollback_result' => CSV_Import_Backup_Manager::rollback_import($session_id)];
            }
        }
        return [];
    }
    ...
    private function handle_profile_form(): array {
         ...
         // Weitere Aktionen (load, delete) hier hinzufügen
         return [];
    }
    
    private function handle_logs_form(): array {
        if (wp_verify_nonce($_POST['_wpnonce'] ?? '', 'csv_import_clear_logs') && class_exists('CSV_Import_Error_Handler')) {
    ...
    private function prepare_logs_data(): array {
        ...
        $filtered_logs = ($filter_level !== 'all') 
            ? array_filter($all_logs, fn($log) => ($log['level'] ?? '') === $filter_level) 
            : $all_logs;
    ...
    private function prepare_debug_data(): array {
        ...
        $data['locks'] = $wpdb->get_results( 
            "SELECT option_name, option_value FROM {$wpdb->options} 
             WHERE option_name LIKE '%csv_import%lock%' OR option_name LIKE '%csv_import_running%'" 
        );
        
        if ( $wpdb->get_var( "SHOW TABLES LIKE '{$wpdb->prefix}actionscheduler_actions'" ) ) {
            $data['stuck_jobs'] = $wpdb->get_results(
                "SELECT hook, status, scheduled_date_gmt FROM {$wpdb->prefix}actionscheduler_actions 
    ...
    public function enqueue_admin_assets(string $hook_suffix): void {
        ...
        // Spezifische Assets für SEO-Seite
